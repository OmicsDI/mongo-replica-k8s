...
kubectl exec mongod-0 -c mongod-container -- mongo --eval 'rs.initiate({_id: "omicsrs", version: 1, members: [ {_id: 0, host: "mongod-0.mongodb-service.default.svc.cluster.local:27017"}, {_id: 1, host: "mongod-1.mongodb-service.default.svc.cluster.local:27017"}, {_id: 2, host: "mongod-2.mongodb-service.default.svc.cluster.local:27017"} ]});'
...
echo "Creating user: 'omicsadmin'"
kubectl exec mongod-0 -c mongod-container -- mongo --eval 'db.getSiblingDB("admin").createUser({user:"omicsadmin",pwd:"'"${1}"'",roles:[{role:"root",db:"admin"}]});'
echo
